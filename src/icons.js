...
import {
  ...
  RotateCcw, RefreshCcwDot, Bot, MessageCircle, UserCog, LayoutDashboard,
  ...
  Send, PanelLeft, Gauge, Activity,
} from "lucide";
import opencodeSymbol from "./assets/opencode-symbol.svg";
...
// Marcas de terceros: Lucide no incluye logos de marca propios ni de terceros.
// Los paths de OpenAI y Claude se toman de Simple Icons (CC0).
// OpenCode conserva el SVG para fondo claro incluido en su kit oficial.
// El isotipo vectorial de Jintia conserva detalle y contraste incluso a 18 px.
const BRAND_ICONS = {
  jintia: { type: "img", src: "/brand/jintia-mark.svg" },
  opencode: { type: "img", src: opencodeSymbol },
...
const INLINE_STYLE = (size) =>
  `display:inline-block;width:${size}px;height:${size}px;vertical-align:middle;flex-shrink:0`;

/** Genera un logo de marca inline. Soporta SVG path (currentColor) e img. */
export function brandIcon(name, size = 14) {
  const def = BRAND_ICONS[name];
  if (!def) return "";
  if (typeof def === "object" && def.type === "img") {
    return `<img src="${def.src}" width="${size}" height="${size}" alt="" aria-hidden="true" style="${INLINE_STYLE(size)};object-fit:contain">`;
  }
  return `<svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="currentColor" aria-hidden="true" style="${INLINE_STYLE(size)}">${def}</svg>`;
}
