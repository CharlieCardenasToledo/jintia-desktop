...
const BRAND_ICONS = {
  jintia: { type: "img", src: "/brand/jintia-chatbot.svg" },
...
/** Genera un logo de marca inline. Soporta SVG path (currentColor) e img (grayscale). */
export function brandIcon(name, size = 14) {
  const def = BRAND_ICONS[name];
  if (!def) return "";
  if (typeof def === "object" && def.type === "img") {
    return `<img src="${def.src}" width="${size}" height="${size}" alt="" aria-hidden="true" style="${INLINE_STYLE(size)};object-fit:contain;filter:grayscale(1)">`;
  }
