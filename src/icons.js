...
/** Genera un logo de marca como SVG inline con fill="currentColor". Sin fondo, adaptable al tema. */
export function brandIcon(name, size = 14) {
  const def = BRAND_ICONS[name];
  if (!def) return "";
  const viewBox = (typeof def === "object" && def.viewBox) ? def.viewBox : "0 0 24 24";
  const path = (typeof def === "object" && def.path) ? def.path : def;
  return `<svg viewBox="${viewBox}" width="${size}" height="${size}" fill="currentColor" aria-hidden="true" style="${INLINE_STYLE(size)}">${path}</svg>`;
}
