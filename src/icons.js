...
// Marcas de terceros (ChatGPT/OpenAI, Claude/Anthropic): Lucide no incluye
// logos de marca, así que estos paths se toman literalmente de Simple Icons
// (CC0) en vez de reconstruirse de memoria, para no distorsionar el isotipo.
const BRAND_ICONS = {
  jintia: '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/>',
...
/** Genera un logo de marca (ChatGPT/Claude) como <svg> inline, fuera del set de Lucide. */
export function brandIcon(name, size = 14) {
  const path = BRAND_ICONS[name];
  if (!path) return "";
  return `<svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="currentColor" aria-hidden="true" style="display:inline-block;width:${size}px;height:${size}px;vertical-align:middle;flex-shrink:0">${path}</svg>`;
}
