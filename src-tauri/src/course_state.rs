...
pub fn read(project_path: String) -> Value {
    ...
    {
        Some(state) => {
            serde_json::json!({ "success": true, "exists": true, "path": state_path.to_string_lossy(), "state": state })
        }
        None => {
            serde_json::json!({ "success": false, "exists": true, "message": "El archivo .jintia/state.json no contiene JSON válido." })
        }
    }
}
